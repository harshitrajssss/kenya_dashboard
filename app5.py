...
def extract_data(markdown_text: str, territory: str):
    ...
    exec_first_line = re.split(r"(?<=[.!?])\s+", exec_match.group(1).strip())[0] if exec_match else ""
    ...
    table = {
        ...
        "Summary": ["\n\n".join(insights) if insights else "USHINDI Laundry Bar faces significant challenges and opportunities in the Nairobi territory"],
    }
    meta = {
        "sku_clusters": list(set(sku_clusters)),
        "executive_summary": exec_first_line,
        "high_potential_regions": high_regions,
    }
    return table, meta
...
default_brand_idx = brand_options.index("USHINDI BAR") if "USHINDI BAR" in brand_options else 0

with st.sidebar:
    st.header("🔎 Filters")
    territory = st.selectbox("Territory", GT_DATA["Markets"].unique())
    brand = st.selectbox("Brand", brand_options, index=default_brand_idx)
