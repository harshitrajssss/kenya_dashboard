...
# ---------------------------------------------------------------------
st.set_page_config(page_title="Territory–Brand Opportunity Dashboard",
                   layout="wide",
                   page_icon="📊")
...
st.markdown(
    ...
    html, body, [data-testid="stApp"] {{background:{NAVY_BG};color:#E3E8EF;font-family:'Segoe UI',sans-serif;}}
    section[data-testid="stSidebar"] > div {{background:{NAVY_BG};}}
    .metric-card {{border:1px solid #334F6B;border-radius:12px;background:{PANEL_BG};padding:1.25rem 1rem;height:220px;overflow-y:auto;}}
    .metric-card h5 {{margin:0 0 .5rem 0;font-size:1rem;color:{ACCENT};}}
    .metric-card p {{margin:0;font-size:.95rem;line-height:1.35rem;}}
    .stDataFrame, .stTable {{background:{PANEL_BG};}}
    .stTable tbody td {{white-space:pre-wrap !important;line-height:1.35rem;}}
...
def parse_markdown(md: str):
    """Extract executive summary, insights, white-space scores, etc."""
    # Full executive-summary block
    exec_block = re.search(r"## Executive Summary\s*(.*?)\n---", md, re.DOTALL)
    executive_summary = exec_block.group(1).strip() if exec_block else ""
    ...
    high_regions_block = re.search(r"### Top high potential Regions\s*(.*?)(?=\n##|\n---)",
                                   md, re.DOTALL)
    high_regions = [r.strip() for r in re.split(r",|;", high_regions_block.group(1))] \
                   if high_regions_block else []

    return {
        "executive_summary": executive_summary,
        "insights": "\n\n".join(insights) if insights else executive_summary,
        "white_space_scores": list(set(white_space_scores)),
        "client_shares": list(set(client_shares)),
        "high_regions": high_regions
    }

def average_white_space(df: pd.DataFrame, terr: str, br: str) -> float:
    subset = df[(df["Markets"] == terr) & (df["brand"] == br)]
...
with st.sidebar:
    ...
    territory = st.selectbox("Territory", territory_opts,
                             index=territory_opts.index("NAIROBI")
                                   if "NAIROBI" in territory_opts else 0)

    brand_opts = list(GT_DATA["brand"].unique())
    brand = st.selectbox("Brand", brand_opts,
                         index=brand_opts.index("USHINDI BAR")
                               if "USHINDI BAR" in brand_opts else 0)
